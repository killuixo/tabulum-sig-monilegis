...
export default function App() {
  ...
  const getMacroSituacao = (situacao) => {
    ...
    if (s.includes('arquivad') || s.includes('rejeitad') || s.includes('retirad')) return 'Encerrados';
  ...
  // =======================================================================
  // CÁLCULOS DO DASHBOARD E LISTAS DE FILTROS
  // =======================================================================
  ...
  const { filteredData, dashboardStats, filterOptions } = useMemo(() => {
    let filtered = data.filter(item => {
      ...
      // Filtro de Aba (Processo x Atividade)
      ...
      if (toggleAprovadas && !sitLower.includes('lei') && !sitLower.includes('norma jurídica')) return false;
    ...
    // Construção das Opções para os Filtros Avançados baseados nos dados da aba atual
    ...
    // Construção das Estatísticas do Dashboard baseadas nos dados FILTRADOS atualmente
    const stats = {
        ...
        {!loading && !error && viewMode === 'card' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredData.map((item, index) => {
              ...
              const sitLower = (getSituacao(item) || '').toLowerCase();
              const isAprovadoLei = sitLower.includes('lei') || sitLower.includes('norma jurídica');
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-6 border-[3px] border-black border-dashed p-4">
                    {(() => {
                      ...
                      const isArquivadoModal = sitLowerModal.includes('arquivad') || sitLowerModal.includes('veto') || sitLowerModal.includes('lei') || sitLowerModal.includes('norma jurídica') || sitLowerModal.includes('retirado') || sitLowerModal.includes('rejeitado');
                    ...
                    return (
                      <div className="pt-4 border-t-[3px] border-black border-dashed flex flex-wrap gap-3">
                        {parsedLinks.map((l, i) => {
                          ...
                          const isAprovada = /\blei\b/.test(labelLower);
                          const btnClass = isAprovada 
                            ? 'bg-[#008080] text-white border-black hover:bg-[#006666]' 
                            : 'bg-black text-white border-black hover:bg-gray-800';
                          
                          return (
                            <a key={i} href={l.url} target="_blank" rel="noreferrer" className={`text-xs font-black uppercase tracking-wider border-[3px] px-4 py-2 flex items-center gap-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-transform hover:-translate-y-1 ${btnClass}`}>
                              {isAprovada && <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><polyline points="20 6 9 17 4 12"/></svg>}
                              {isAprovada ? 'LEI APROVADA' : l.label}
                            </a>
                          );
                        })}
                      </div>
